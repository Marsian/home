...
OUT_DIR = Path("src/game-center/pixel-knight/assets/village/v7-front/atoms")
...
def main() -> None:
    ...
    (OUT_DIR / "atoms.json").write_text(json.dumps([asdict(a) for a in atoms], ensure_ascii=False, indent=2) + "\n")

    # Also emit a TS manifest so runtime can preload & draw atoms via stable imports.
    lines: list[str] = []
    lines.append("// Generated by scripts/slice-pixel-knight-v7-village-cutouts.py")
    lines.append("export type StarterVillageV7AtomId =")
    for atom in atoms:
        lines.append(f"  | {json.dumps(atom.id)}")
    lines.append("")
    for atom in atoms:
        var = atom.id.replace("-", "_")
        lines.append(f"import {var}Url from './{atom.file}'")
    lines.append("")
    lines.append("export const starterVillageV7Atoms = [")
    for atom in atoms:
        var = atom.id.replace("-", "_")
        x, y, w, h = atom.sheet_bbox
        lines.append(
            f"  {{ id: {json.dumps(atom.id)} as const, src: {var}Url, width: {w}, height: {h} }},"
        )
    lines.append("] as const")
    lines.append("")
    (OUT_DIR / "atomsManifest.ts").write_text("\n".join(lines) + "\n")

    print(f"Wrote {len(atoms)} atoms to {OUT_DIR} (plus atomsManifest.ts)")
