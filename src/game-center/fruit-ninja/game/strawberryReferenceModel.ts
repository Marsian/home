...
import strawberryReferenceUrl from '../assets/glb/strawberry.glb?url'
...
type PreparedStrawberryReference = {
...
function prepareReferenceMesh(sourceMesh: THREE.Mesh): PreparedStrawberryReference | null {
...
export function createStrawberryReferenceMesh(radius: number): THREE.Group | null {
  ...
  const targetHeight = radius * 2.18
...
export function createStrawberryReferenceHalfMesh(
  radius: number,
  half: 'top' | 'bottom',
...
export function createStrawberryReferenceCapGeometry(radius: number): THREE.ShapeGeometry | null {
  if (!preparedReference) return null
