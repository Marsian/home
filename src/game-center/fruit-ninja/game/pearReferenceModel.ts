...
import pearReferenceUrl from '../assets/glb/pear.glb?url'
...
let preparedReference: PreparedPearReference | null = null
...
function prepareReferenceMesh(sourceMesh: THREE.Mesh): PreparedPearReference | null {
  ...
  const sourceCutY = -scratchSize.y * 0.13
...
export function primePearReferenceModel() {
  ...
  loadPromise = new Promise((resolve) => {
    loader.load(
      ...
      (gltf) => {
        ...
        if (!sourceMesh) {
          console.warn('[fruit-ninja] pear reference GLB had no mesh')
          resolve()
          return
        }
        ...
        if (!preparedReference) {
          console.warn('[fruit-ninja] failed to prepare pear reference mesh')
        }
        resolve()
      },
      undefined,
      (error) => {
        console.warn('[fruit-ninja] failed to load pear reference GLB', error)
        resolve()
      },
    )
  })
...
export function createPearReferenceHalfMesh(
  radius: number,
  half: 'top' | 'bottom',
...
export function createPearReferenceCapGeometry(radius: number): THREE.ShapeGeometry | null {
  if (!preparedReference) return null
